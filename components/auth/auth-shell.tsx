...
export function AuthShell({
  ...
  return (
    <main className="min-h-screen bg-[linear-gradient(180deg,#f7fbfa_0%,#edf6f4_100%)] px-4 py-6 sm:px-6 sm:py-10">
      <div className="mx-auto flex w-full max-w-3xl items-center justify-between pb-6 sm:pb-8">
      ...
      <section className="mx-auto w-full max-w-3xl rounded-[2rem] border border-[#d9e9e4] bg-white p-6 shadow-[0_20px_55px_rgba(22,48,66,0.08)] sm:p-8">
        <div className="mb-6 space-y-3">
          <div className="inline-flex rounded-full border border-[#d9e9e4] bg-[#f7fbfa] px-3 py-1 text-xs font-medium text-[#456473]">
            {badge}
          </div>
          <div className="space-y-2">
            <h1 className="text-2xl font-semibold tracking-tight text-[#163042] sm:text-3xl">
              {title}
            </h1>
            <p className="max-w-2xl text-sm leading-7 text-[#5b7483] sm:text-base">
              {description}
            </p>
          </div>
        </div>

        <div className="mb-6 overflow-hidden rounded-[1.75rem] border border-[#d9e9e4] bg-[linear-gradient(155deg,#163042_0%,#125e8a_56%,#0f9d8a_100%)] p-5 text-white sm:p-6">
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              {highlights.map((item) => (
                <div
                  ...
                  key={item}
                >
                  <CheckCircle2 className="mt-0.5 size-5 shrink-0 text-white/88" />
