...
import type { DashboardPageData } from "../queries"
import { getAlertIcon } from "../utils/alert-icons"
import { formatDate, formatKg, formatNumber } from "../utils/formatters"
import { EmptyState, HighlightCard, MetricRow } from "./dashboard-display"
...
export function DashboardHero({ data }: { data: DashboardPageData }) {
  const quickActions = [
...
export function DashboardSummaryCards({ data }: { data: DashboardPageData }) {
...
export function DashboardAttentionGrid({ data }: { data: DashboardPageData }) {
  return (
    <section className="grid gap-4 xl:grid-cols-[minmax(0,1.08fr)_minmax(19rem,0.92fr)]">
      <DashboardAlerts data={data} />
      <NearestHarvestCard data={data} />
    </section>
  )
}

function DashboardAlerts({ data }: { data: DashboardPageData }) {
  return (
...
function NearestHarvestCard({ data }: { data: DashboardPageData }) {
  return (
    <div className="rounded-[1.75rem] border border-[#d9e9e4] bg-white p-5 shadow-[0_14px_34px_rgba(22,48,66,0.04)] sm:p-6">
      <div className="flex items-start gap-3">
...
export function ActiveCyclesOverview({ data }: { data: DashboardPageData }) {
